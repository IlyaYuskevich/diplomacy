...
var (
	samplePlayer1Json        = `{"id":"d3ee3df0-56cf-43f8-85ff-bcb4efb3d4ad","name":"Luke Skywalker","createdAt":.{20,40}}`
	samplePlayer2Json        = `{"id":"a00935cb-4e9d-409d-814a-e4d704e0e61d","name":"Darth Weider","createdAt":.{20,40}}`
	samplePlayer3Json        = `{"id":"675c0b41-23ea-4833-ac9d-5182b0df0605","name":"Boba Fett","createdAt":.{20,40}}`
	samplePlayerJsonPayload1 = `{"name":"Chewbacca"}`
	samplePlayerJsonPayload2 = `{"name":"Han Solo"}`
	samplePlayer1JsonPatched = `{"id":"d3ee3df0-56cf-43f8-85ff-bcb4efb3d4ad","name":"Han Solo","createdAt":.{20,40}}`
)
...
func TestGetPlayers(t *testing.T) {
	...
	if assert.NoError(t, h(ctx)) {
		...
		assert.Regexp(t, samplePlayer1Json, rec.Body.String())
		assert.Regexp(t, samplePlayer2Json, rec.Body.String())
		assert.Regexp(t, samplePlayer3Json, rec.Body.String())
	}
}
...
func TestPatchPlayer(t *testing.T) {
	...
	if assert.NoError(t, h(ctx)) {
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.Regexp(t, samplePlayer1JsonPatched, rec.Body.String())
	}
}
