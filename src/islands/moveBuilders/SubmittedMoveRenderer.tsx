...
export function SubmittedMoveRenderer(
  ...
  const moveFormatter = (move: SubmittedMove | SubmittedMoveInsert) => {
    switch (move.type) {
      ...
      case "RETREAT":
        return `${move.unit_type} retreats ${move.origin}-${move.to}`;
      case "SUPPORT":
  ...
  const deleteSubmittedMove = async (move: SubmittedMove | SubmittedMoveInsert) => {
    if (move.id) { await fetch(`/api/submitted-move/${move.id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
    });
    submittedMoves.value = submittedMoves.value.filter((mv) => mv.id != move.id);
  } else {
    submittedMoves.value = submittedMoves.value.filter((mv) => mv.origin != move.origin);
  }
  };
